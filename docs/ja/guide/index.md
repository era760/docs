---
title: 👣 はじめに
description: ブリッジを始めるにはどこから手をつければいいか、学んでみましょう！
sidebar: Guide
nav_order: 0
---
...
bridge. を初めて使う方でも、アドオン全般に不慣れな方でも、このガイドはbridge. の設定や使い方に慣れるための入門書として役立ちます。このページは順番に読んでいただくことを想定していますが、すでに知っている内容があれば、次のセクションへ進んでいただいても構いません。

このガイドでは、以下のことを学びます:

:ballot_box_with_check: bridge. の設定方法<br/>
:ballot_box_with_check: プロジェクトの設定方法<br/>
:ballot_box_with_check: bridge. を使ってアドオンを作成する最初のステップ<br/>

## bridge. の設定

bridge. は、**プログレッシブ・ウェブ・アプリ（PWA）** として設計されているため、多くのプラットフォームでさまざまな方法で利用できます。つまり、デスクトップのChromeやEdge、AndroidやiOSのChromeやSafariなど、最新のブラウザを搭載したほぼすべてのデバイスでbridge. を利用できるということです。

お使いのブラウザが対応している場合は、[デバイスへのbridge.のインストール方法に関するガイド](/ja/guide/download/index)を別途ご用意しています。また、ブラウザから直接 https://editor.bridge-core.app/ にアクセスして bridge. をご利用いただくことも可能です。

:::tip
なお、このインストールは、従来の `.exe` ファイルによるインストールとは異なりますが、インストール後は bridge. がネイティブアプリと同様に動作し、オフラインでも利用可能です。
:::

残りの設定手順は、お使いのブラウザによって異なる場合があります。次の手順をよくお読みいただき、画面に表示されない場合はその手順をスキップしてください。

1. [`com.mojang` フォルダ](/ja/guide/misc/com-mojang-syncing/index) をbridge. にリンクします。デバイスのファイルエクスプローラーから、エディタの上にドラッグして移動させてください。
